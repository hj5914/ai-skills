...
6. 如果想减少手工复制模板，可直接用 `python3 tools/bdo.py` 驱动本地产物生成；`classify` 支持重复传入 `--surface` 标记改动面，`delta` 会自动生成结构化 summary，`contract` 会据此预填默认块，`verify` 支持 `--evidence` / `--gap` 并把真实验证证据写入 state，`handoff` 会优先消费最新 delta 和验证证据，`memory` 支持 `--context` / `--lesson` / `--rule` / `--evidence` 并把交付经验回写到 state，state 写入采用原子替换避免半写入 JSON。
